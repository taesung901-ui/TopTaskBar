...
- 다른 창을 최소화한 직후 Windows가 뒤의 창을 자동 활성화하면, 3초 이내 첫 클릭은 최소화하지 않고 전면 배치만 수행
...
실행 중인 앱을 우클릭해 추가할 때 웨일의 버전별 실행 경로는 안정적인 상위 실행 파일 경로로 보정한다. Hyper-V 관리자는 공용 호스트인 `mmc.exe`만 저장하지 않고 `virtmgmt.msc` 콘솔 파일을 실행 인수로 함께 저장한다. 식별하지 못한 다른 MMC 창은 잘못된 항목 생성을 막기 위해 자동 추가하지 않는다.
